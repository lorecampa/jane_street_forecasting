...
class JaneStreetMultiStockGraphModel(L.LightningModule):
    ...
    def __init__(self, 
        ...
        super(JaneStreetMultiStockGraphModel, self).__init__()   
        ...
        self.acc_metrics = dict(ss_res=0, ss_tot=0, abs_err_sum=0, weights_sum=0)

    def forward(self, x, symbols, adj_matrix):
        return self.model(x, symbols, adj_matrix)

    def training_step(self, batch, batch_idx):
        x, y, mask, weights, symbols, adj = batch
        y_hat = self.forward(x, symbols, adj).squeeze(-1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        metrics = dict()
        metrics['val_wmse'] = ss_res_step / weights_sum_step
        metrics['val_wmae'] = abs_err_step / weights_sum_step
        metrics['val_wr2'] = 1 - ss_res_step / ss_tot_step if ss_tot_step > 0.0 else 0.0
