...
def sample_catboost_params_online(trial: optuna.Trial, additional_args: dict = {}) -> dict:
...
def train_with_es(init_model: CatBoostRegressor, params: dict, train_dl, val_dl, use_weighted_loss, output_dir, es_patience):
    ...
    logging.info(f"Learning rate: {_params['learning_rate']:.4f}")
...
def optimize_parameters(output_dir, pretrain_dataset: pl.LazyFrame, pretrain_val_dataset: pl.LazyFrame, online_learning_dataset: pl.LazyFrame, features: list, study_name, n_trials, storage, start_eval_from):   
    def obj_function(trial):
        ...
        params = sample_catboost_params_online(trial, additional_args={'use_gpu': torch.cuda.is_available()})
        ...
        for i in range(0, len(date_ids), batch_size):
            ...
            for date_id, test in tqdm(batch_online_learning_dataset.group_by('date_id', maintain_order=True), total=n_batch_days):
                ...
                if (date_idx + 1) % train_every == 0:                
                    ...
                    params['learning_rate'] = max(params['learning_rate'] * lr_decay, 1e-5)
            ...
            del batch_online_learning_dataset
            gc.collect()
...
def main(dataset_path, output_dir, study_name, n_trials, storage):
    ...
    pretraining_dataset = loader.load_with_partition(7, 8)
    ...
    # online_learning_dataset = loader.load(1151, 1250)
...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = args.output_dir or EXP_DIR / 'catboost_online'
    ...
    model_name = f'catboost_online_tuning_{timestamp}'
