...
def optimize_parameters(output_dir, train_dataset, val_dataset, study_name, n_trials, storage, 
    ...
    def obj_function(trial):
        ...
        batch_size = trial.suggest_categorical("batch_size", [4096, 8192, 16384, 32768, 65536, 131072])
        ...
        final_hidden_dims = trial.suggest_int("final_hidden_dims", 16, 256, step=16)
        hidden_dims = [final_hidden_dims]
        
        if num_layers > 1:
            hidden_dims_decay = trial.suggest_float("hidden_dims_decay", 1, 3, step=0.1)
            for _ in range(num_layers - 1):
                hidden_dims = [int(hidden_dims[0] * hidden_dims_decay)] + hidden_dims
...
if __name__ == '__main__':
    ...
    model_name = f'mlp_tuning_{timestamp}'
