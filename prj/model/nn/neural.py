...
class TabularNNModel(ABC):
    ...
    def fit(
        ...
        metrics: List[Union[str, tfk.metrics.Metric]] = [],
        ...
        print('Fitted data preprocessors')
        if free_raw_data:
            del X
            gc.collect()
        ...
        if validation_data is not None:
            ...
            validation_data = (self._transform_test_data(X_val), y_val, sample_weight_val)
            if free_raw_data:
                del X_val
                gc.collect()
        ...
        print(f'Fit complete after {len(fit_history["loss"])}')
    ...
    def _build_input_layers(self):
        ...
        if len(self.numerical_features) > 0:
            key = 'NumericInput'
            numeric_input_layer = tfkl.Input(shape=(len(self.numerical_features),), name=key)
        ...
        if self.categorical_features is not None:
            if self.categorical_transform == 'embeddings':
                ...
                for feature in self.categorical_features:
                    ...
                    cat_input = tfkl.Input(shape=(1,), name=key)
