...
def optimize_parameters(model_file_path: str, y_pred_offline, output_dir, online_old_dataset: pl.LazyFrame, online_learning_dataset: pl.LazyFrame, features: list, study_name, n_trials, storage):   
    ...
    if pruner_name == 'median':
        pruner = optuna.pruners.MedianPruner(n_startup_trials=10, n_warmup_steps=30, interval_steps=1)
    else:
        pruner = optuna.pruners.NopPruner()
    
    study = optuna.create_study(directions=['maximize', 'maximize'], study_name=study_name, storage=storage, pruner=pruner, load_if_exists=True)    
