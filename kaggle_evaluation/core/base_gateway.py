...
class BaseGateway():
    ...
    def write_submission(self, predictions):
        ...
        if isinstance(predictions, pd.DataFrame):
            pl.DataFrame(predictions).to_parquet('submission.parquet', index=False)
    ...
    def handle_server_error(self, exception: Exception, endpoint: str):
        ...
        if 'Exception calling application' in exception_str:
            raise GatewayRuntimeError(GatewayRuntimeErrorType.SERVER_RAISED_EXCEPTION, exception_str) from None
